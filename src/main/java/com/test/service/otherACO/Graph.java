...
public class Graph implements Iterable<Vertex> {
    ...
    public static   int[][] distances; //距离
     ...
     */
    public Graph(double evaporationRate, int alpha, int beta, int[][] distances) {
    ...
    public void addVertex (Vertex vertex) {
        hashMap.put(Integer.valueOf(vertex.name), vertex);
        list.add(vertex);
    }

    public Vertex getVertex (Node node) {
       // System.out.println(node.hashCode());
        return hashMap.get(Integer.valueOf(node.name));
    }
    ...
    public static double getDistance(Node node1, Node node2) {
        int dis = distances[Integer.valueOf(node1.getName())][Integer.valueOf(node2.getName())];
