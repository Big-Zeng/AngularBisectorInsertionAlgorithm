...
public class Population implements Iterable<Chromosome> {
    ...
    public static Population fromDataSet(int popSize, List<UserAllocation> userAllocations, Random r) {
        City[] cities = new City[userAllocations.size()];
        for (int i = 0; i < userAllocations.size(); i++) {
            UserAllocation user= userAllocations.get(i);
            cities[i] = new City(String.valueOf(user.getAreaInnerId()), (int) user.getUserLocation().x, (int) user.getUserLocation().y);
        }
       // City[] cities = Import.getCities(dataSet);
